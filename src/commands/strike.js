...
const { ApplicationCommandOptionType, PermissionFlagsBits, EmbedBuilder, GuildMemberRoleManager, GuildMember } = require('discord.js');
...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        } else if (!(targetUser.roles.cache.has(strike2)) && targetUser.roles.cache.has(strike1)) {
        ...
        } else if (!(targetUser.roles.cache.has(strike3)) && targetUser.roles.cache.has(strike2) && targetUser.roles.cache.has(strike1)) {
        ...
        try {
            await interaction.editReply({content: `<@${targetUserId}>`, embeds: [strikeEmbed]});
        } catch (error) {
